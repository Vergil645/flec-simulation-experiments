...
parser.add_argument("-m", help='plot method: can be "CDF", "uni", uni_favorable_path or "scatter"', default="uni")
parser.add_argument("--multiplex-metric", help='name of the metric on which to multiplex the data: one curve/scatter for each value of this metric', default=None)
parser.add_argument("--multiplex-metric-in-bytes", action="store_true", default=False, help='if set, the --cdf-multiplex-metric should be interpreted as bytes and the labels updated accordingly')
...
parser.add_argument("--rwin-size", help="specifies the rwin size(s) to look at to plot the points", default="")
parser.add_argument("--normalize-bytes-sent", help="if present, divides", action="store_true")
...
for i, (filename, filename2, filename3, METHOD_1, METHOD_2, METHOD_3) in enumerate(zip(files, files2, files3, name1, name2, name3)):
    ...
    def plot_uni(y_axes, x_axes, transform, marker="+", color=None, label=None, linestyle=None, log=False):
        ...
        if transforms_output[transform] > 1 or color is None or label is None:
            ...
            plt.plot(x_axes, [math.log(n, 10) for n in y_axes[0]] if log else y_axes[0], color=color, linewidth=linewidth, marker=marker, linestyle="-" if linestyle is None else linestyle,
    ...
    if filename[-5:] == ".json":
        ...
        def plot(filesize, metric, marker=None, color=None, label=None, linestyle=None):
            if c2 is None:
                if plot_method == "CDF":
                    command = "SELECT {0}_{1}, {0}_{2} {4} FROM results WHERE bw != 0 {3} AND {0}_{1} != -1 AND {0}_{2} > 0"\
                        .format(metric, METHOD_1, METHOD_2, ("AND file_size == %d" % filesize) if filesize is not None else "", (", " + args.multiplex_metric) if args.multiplex_metric else "")
                else:
                    rwin_sizes = args.rwin_size.split(",")
                    rwin_queries = ["stream_receive_window_size = {}".format(x) for x in rwin_sizes]
                    command = "SELECT {0}_{1}, {0}_{2}, {4} {5} FROM results WHERE bw != 0 {3} AND {0}_{1} != -1 AND {0}_{2} > 0 AND {6}"\
                        .format(metric, METHOD_1, METHOD_2, ("AND file_size == %d" % filesize) if filesize is not None else "", ", ".join(args.uni_x_metric.split(",")), (", " + args.multiplex_metric) if args.multiplex_metric else "", " OR ".join(rwin_queries))
                c.execute(command)
                res = c.fetchall()
            ...
            if plot_method == "CDF":
                ...
                if len(metric_vals) > 0 and len(metric_vals[0]) == 1 and transforms_output[transform] > 1:
                    metric_vals = [v*transforms_output[transform] for v in metric_vals]
                to_plot = [[r[i] for r in transformed_result] for i in range(transforms_output[transform])]
                x_axis = [[(v[i]/float(args.filesize)) if args.normalize_bytes_sent else v[i] for v in metric_vals] for i in range(transforms_output[transform])]
                if args.transform == "none":
                    if args.labelfirsttest is not None and args.labelsecondtest is not None:
        ...
        for meth_idx, method_1 in enumerate(methods_1):
            ...
            for metric_idx, metric in enumerate(metrics):
                try:
                    ...
                    plot(filesize, metric, marker=markers[meth_idx + metric_idx + i] if len(methods_1) > 1 else None, color=colors[meth_idx + metric_idx + i], label=legends[meth_idx + metric_idx + i] if legends is not None else None)
                except ValueError:
